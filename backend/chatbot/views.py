    ...
    def post(self, request):
        ...
        try:
            ...
            parameters = json.loads(
                dialogflow.types.struct_pb2.Struct.SerializeToJsonString(response.query_result.parameters)
            )
            ...
            output_serializer = ChatbotResponseSerializer({
                ...
                'context': {
                    c.name.split('/')[-1]: json.loads(
                        dialogflow.types.struct_pb2.Struct.SerializeToJsonString(c.parameters)
                    )
                    for c in output_contexts
                },
                'intent': intent,
                'parameters': parameters,
                'suggestions': []  # adjust if you use suggestions
            })
